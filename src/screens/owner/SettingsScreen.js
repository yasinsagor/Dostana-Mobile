...
export default function OwnerSettingsScreen() {
  ...
  const [open, setOpen] = useState({ reports:false, supplier:false, branches:true, managers:false, data:false, automation:false, partners:false, app:false, exports:false, audit:false });
  ...
  // Audit log — last 30 submissions across all types
  const auditLog = [
    ...
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.content}>
        ...
        <View style={s.profile}>
          <View style={s.crown}><Text style={{fontSize:28}}>👑</Text></View>
          <View style={{flex:1}}>
            <Text style={s.profileName}>Owner</Text>
            <Text style={s.profileSub}>Full access · {BRANCHES.length} branches · Dostana Kebab</Text>
          </View>
          <TouchableOpacity style={s.logoutBtn} onPress={()=>Alert.alert('Log Out','Are you sure?',[{text:'Cancel',style:'cancel'},{text:'Log Out',style:'destructive',onPress:logout}])}>
        ...
        {open.data&&(
          <View style={s.sectionBody}>
            <ToolBtn icon="🔄" label="Sync with Supabase" sub="Force refresh all data from server" bg="#E0F2F1" color="#00695C" onPress={()=>{ setLoading(true); load(); Alert.alert('Synced','Data refreshed from Supabase.'); }}/>
        ...
        {/* ── 8. APP MANAGEMENT ── */}
        <SectionHeader icon="📱" title="App Management" color="#37474F" open={open.app} onPress={()=>toggle('app')}/>
        {open.app&&(
          <View style={s.sectionBody}>
            {[
              ['App Name','Dostana Mobile'],['Version','1.0.0'],['Backend','Supabase'],
              ['Branches',String(BRANCHES.length)],['Environment','Production'],
            ].map(([k,v])=>(
            ...
            <ToolBtn icon="🔄" label="Force Refresh" sub="Clear cache and reload all data" bg="#ECEFF1" color="#37474F" onPress={()=>{ setLoading(true); load(); }}/>
            <ToolBtn icon="🌙" label="Theme" sub="Light / Dark mode (coming soon)" bg="#ECEFF1" color="#37474F" onPress={()=>soon('Theme')}/>
          </View>
        )}
...
const s = StyleSheet.create({
  ...
  profile:{flexDirection:'row',alignItems:'center',backgroundColor:COLORS.primary,borderRadius:16,padding:16,gap:14,marginBottom:8},
  crown:{width:52,height:52,borderRadius:26,backgroundColor:'rgba(255,255,255,0.2)',alignItems:'center',justifyContent:'center'},
  profileName:{fontSize:18,fontWeight:'800',color:'#fff'},
  profileSub:{fontSize:11,color:'rgba(255,255,255,0.8)',marginTop:2},
