...
function fmtK(n) { if (!n) return '0'; if (Math.abs(n) >= 1000) return (n / 1000).toFixed(1) + 'k'; return Math.round(n).toString(); }
...
export default function OwnerDashboardScreen() {
  ...
  const alerts = [];
  if (missingDaily.length > 0) alerts.push({ kind: 'error', icon: '🚨', msg: `${missingDaily.length} missing daily report: ${missingDaily.map(b => b.name).join(', ')}` });
  if (missingCF.length > 0) alerts.push({ kind: 'warn', icon: '⚠️', msg: `Cash flow missing: ${missingCF.map(b => b.name).join(', ')}` });
  if (todaySpec.length === 0) alerts.push({ kind: 'warn', icon: '📦', msg: 'No SPEC orders submitted today' });
  if (negBranches.length > 0) alerts.push({ kind: 'error', icon: '📉', msg: `Negative balance: ${negBranches.map(r => r.branch).join(', ')}` });
  if (todayRevenue > 0 && todayExpenses / todayRevenue > 0.8) alerts.push({ kind: 'warn', icon: '💸', msg: `High expenses: ${Math.round(todayExpenses / todayRevenue * 100)}% of today revenue` });
          ...
          <View style={{ gap: 8, marginBottom: 4 }}>
            {alerts.map((a, i) => (
              <View key={i} style={[s.alertCard, a.kind === 'error' ? s.alertError : s.alertWarn]}>
                ...
                <Text style={[s.alertText, a.kind === 'error' ? { color: '#B71C1C' } : { color: '#E65100' }]} numberOfLines={2}>{a.msg}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        <View style={s.card}>
          {todaySpec.length === 0 ? (
            ...
            <>
              ...
              {topBranch && <View style={s.specHighlight}><Text style={s.specHighlightTxt}>🏆 {topBranch} ordered the most today</Text></View>}
            </>
          )}
        </View>
          ...
          return (
            <View key={b.name} style={[s.branchCard, !b.submitted && { opacity: 0.6 }]}>
              <View style={s.branchHeader}>
                ...
                <View style={s.badges}>
                  <View style={[s.badge, b.submitted ? s.badgeGreen : s.badgeRed]}><Text style={s.badgeTxt}>{b.submitted ? '✅ Daily' : '❌ Daily'}</Text></View>
                  <View style={[s.badge, b.specDone ? s.badgeGreen : s.badgeGrey]}><Text style={s.badgeTxt}>{b.specDone ? '✅ SPEC' : '— SPEC'}</Text></View>
                </View>
              </View>
              {b.submitted && (
                <View style={s.branchStats}>
                  <View style={s.bStat}><Text style={s.bStatVal}>{fmtK(b.revenue)}</Text><Text style={s.bStatLbl}>Revenue</Text></View>
                  <View style={s.bStat}><Text style={[s.bStatVal, { color: b.profit >= 0 ? COLORS.primary : COLORS.danger }]}>{b.profit >= 0 ? '+' : ''}{fmtK(b.profit)}</Text><Text style={s.bStatLbl}>Profit</Text></View>
                  <View style={s.bStat}><Text style={s.bStatVal}>{b.hours}h</Text><Text style={s.bStatLbl}>Hours</Text></View>
                </View>
              )}
            </View>
          );
            ...
            {trendByDay.map((d) => {
              ...
              return (
                <View key={d.date} style={s.barCol}>
                  ...
                  <Text style={[s.barLbl, isToday && { color: COLORS.primary, fontWeight: '700' }]}>{dayLabel(d.date)}</Text>
                </View>
              );
            })}
          </View>
...
const s = StyleSheet.create({
  ...
  pageDate: { fontSize: 13, color: '#888' },
  ...
  metCard: { width: '47.5%', backgroundColor: '#fff', borderRadius: 14, padding: 16, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  metLabel: { fontSize: 11, fontWeight: '700', color: '#aaa', letterSpacing: 1, marginBottom: 6 },
  metVal: { fontSize: 22, fontWeight: '800' },
  card: { backgroundColor: '#fff', borderRadius: 14, padding: 16, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
  ...
  branchStats: { flexDirection: 'row', marginTop: 10, paddingTop: 10, borderTopWidth: 1, borderTopColor: '#f0f0f0' },
  ...
  actBtn: { width: '47.5%', backgroundColor: '#fff', borderRadius: 14, padding: 18, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4, elevation: 2 },
