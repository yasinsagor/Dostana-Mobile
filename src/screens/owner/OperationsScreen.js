...
export default function OwnerOperationsScreen() {
  ...
  const [open, setOpen] = useState({ reports:false, supplier:false, managers:false, partners:false, exports:false, audit:false });
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.pageHeader}>
        ...
        <Text style={s.pageSub}>Reports · Supplier · Performance · Exports</Text>
      </View>
            ...
            {mgr30.sort((a,b)=>b.count-a.count).map(m=>(
              <View key={m.name} style={s.mgrCard}>
                <View style={{flexDirection:'row',alignItems:'center',marginBottom:6}}>
                  <View style={[s.dot,{backgroundColor:m.count>=20?COLORS.primary:m.count>=10?COLORS.warning:COLORS.danger}]}/>
...
const s = StyleSheet.create({
  ...
  partnerSub:  { fontSize:12, color:'#888', lineHeight:18, marginBottom:10 },
